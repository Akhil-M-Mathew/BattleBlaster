...
void AProjectile::BeginPlay()
{
	...
	if (LaunchSound)
	{
		UGameplayStatics::PlaySoundAtLocation(GetWorld(), LaunchSound, GetActorLocation(), 0.25f);
	}
	...
	FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (AActor* ProjectileOwner = GetOwner())
	{
		...
		if (OtherActor && OtherActor != this && OtherActor != ProjectileOwner)
		{
			...
			if (HitSound)
			{
				UGameplayStatics::PlaySoundAtLocation(GetWorld(), HitSound, GetActorLocation(), 1.0f, 2.0f);
			}
			if (HitShakeClass)
			{
				...
				if (PlayerController)
				{
					PlayerController->ClientStartCameraShake(HitShakeClass);
				}
			}
		}
	}
